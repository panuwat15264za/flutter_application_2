...
app.post("/signin", async (req, res) => {
   try {
      ...
      const sql = `SELECT COUNT(*) AS count FROM signin WHERE username = "${username}" AND password = "${password}" AND email = "${email}"`;
...
app.post("/signup", async (req, res) => {
   ...
   const password = req.body.password;
   console.log(req);
   let sql = "CREATE TABLE IF NOT EXISTS signin (id INT AUTO_INCREMENT PRIMARY KEY, reg_date TIMESTAMP default CURRENT_TIMESTAMP,name VARCHAR(255), username VARCHAR(255),phone VARCHAR(255),email VARCHAR(255),password VARCHAR(100),img VARCHAR(100))";
...
app.post("/getDB", async (req,res) => {
    ...
    let sql = `SELECT name ,username, email, phone,password FROM ${tablename} WHERE username = '${req.body.username}'`;
    let result = await queryDB(sql);
...
app.get("/showDB", async (req,res) => {
    let sql = `SELECT id, name, username, password, phone, email FROM ${tablename}`;
    let result = await queryDB(sql);
    result = Object.assign({},result);
    console.log(result);
    res.json(result);
